...
```
spec/fixtures/
├── NNN-name.oui            complete-program fixtures (001–090)
...
| 083–090 | THE JS PLAIN-OBJECT MODEL — a plain object literal has a `[[Prototype]]`, and lang-core reads through it with `[]`, `in` and `.`. Runtime-object key order is tree-observable through `toLiteralAST` → `Obj.entries` → the `$ast` entries ARRAY (which the serializer never sorts), so an `@Each`-captured row emits `"2", "10", b, a, "4294967295"` (083). `BUILTINS[node.name]` is a property GET, so `"x" + @toString(1)` finds `Object.prototype.toString`, reads `.fn` (undefined) and throws `builtin.fn is not a function` — the EXPRESSION half of 078, where the prop keeps its raw `$ast` and a `runtimeErrors` entry is recorded (084). `o["__proto__"] = v` does not merely drop the key, it RE-POINTS the prototype: an object-valued one makes the row inherit `k: "StateRef"` so it is EVALUATED (`rows: [7]`), nested one level it yields `value: 1`, and a state declaration serializes as `{"$ast": {"a": 1}}` (085); a component-valued one makes the row serialize AS a TextContent ELEMENT (086); a `null`-valued one leaves the object with no `toString` and no `valueOf`, so `"t" + $s` THROWS ToPrimitive — while the sibling `Object.fromEntries` path (a plain-object literal inside a runtime expression) keeps `__proto__` as an ORDINARY own key and stringifies to `[object Object]` (087). Member access inherits real function values: `.toString`, `.constructor`, `.hasOwnProperty`, `$arr.constructor` through array-pluck, `fn.name`/`fn.length`, `$obj.__proto__.constructor`, an element's own `typeName`/`partial`/`type`, and a function-valued prop DROPPED by `JSON.stringify` (088). `Array.prototype.sort` (ES SortIndexedProperties) partitions `undefined` to the END and never invokes the comparator on it, so `@Sort([3, $obj.nope, 1])` is `"1,3,"` and `@Sort([$badToString, $obj.nope])` raises NO TypeError (089). `@Sort`'s string branch is `localeCompare` = CLDR-root collation with `alternate = non-ignorable`, where hyphen and space are ordinary characters sorting before letters — `"a-b" < "ab"`, `" s" < "1"`, `"-0" < "[object Object]"` (090) |

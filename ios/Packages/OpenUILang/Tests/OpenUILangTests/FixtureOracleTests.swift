...
@Suite struct FixtureOracleTests {
    /// CI gate: exactly 97 fixtures must be discovered, and the exact line
    /// "fixtures exercised: 97" must reach stdout.
    @Test func corpusSize() {
        let fixtures = FixtureCorpus.discover()
        print("fixtures exercised: \(fixtures.count)")
        #expect(
            fixtures.count == 97,
            "expected 97 fixtures under \(FixtureCorpus.fixturesRoot.path), found \(fixtures.count)"
        )
    }

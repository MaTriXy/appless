...
/// (`parser/builtins.js`). `Math.round` is NOT Swift's `rounded()`
...
///
/// Twin of
/// `android/openui-lang/src/test/kotlin/dev/appless/openuilang/MathRoundSemanticsTest.kt`
...
@Suite struct MathRoundSemanticsTests {
    ...
    @Test func roundMatchesECMAScriptMathRound() {
        let result = OpenUIParser(schema: Self.schema).parse(Self.program)
        let actual = TreeSerializer.serialize(result)
        #expect(actual == Self.expected, "@Round diverged from the JS oracle")
    }
}
